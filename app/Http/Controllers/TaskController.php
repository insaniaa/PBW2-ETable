...
class TaskController extends Controller
{
    ...
    public function index(Request $request)
    {
        if ($request->ajax()) {
            ...
            return DataTables::of($tasks->get())
                ...
                ->rawColumns(['actions', 'task_file', 'task_module'])
                ->make(true);
        }
    ...
    public function create()
    {
        try {
            ...
            $studentClasses = StudentClass::whereIn('id_student_class', $classes)->get();
    ...
    public function edit(Task $task)
    {
        try {
            $courses = Course::all();
            $classes = Schedule::where('nip', auth()->user()->lencturer->nip)
                ->distinct()
                ->pluck('id_student_class');
    ...
    public function tasksToday(Request $request)
    {
        ...
        if ($user->hasRole('Student')) {
            ...
            $classes = Schedule::where('nip', $user->lecturer->nip)
                ->distinct()
                ->pluck('id_student_class');

            $schedulesTodayQuery = Schedule::with('course', 'lecturer')
            ->where('schedule_day', $todayInIndonesian) // Use the mapped day
            ->whereIn('id_student_class', $classes)
            ->where('schedule_status', 1);

            // Filter tasks by the student classes associated with the lecturer
            $tasks = $tasks->whereIn('id_student_class', $classes);
            $title = 'Kelola Tugas';
            $classes = StudentClass::whereIn('id_student_class', $classes)->get();
        }
