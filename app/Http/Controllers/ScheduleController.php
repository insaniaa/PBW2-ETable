...
class ScheduleController extends Controller
{
    ...
    public function index(Request $request)
    {
        if ($request->ajax()) {
            ...
            return DataTables::of($schedules)
                ...
                ->editColumn('room.room_name', function ($row) {
    ...
    public function store(StoreScheduleRequest $request)
    {
        ...
        try {
            ...
            if ($existingSchedule) {
                DB::rollBack();
                return redirect()->back()->withInput()->with('error', 'Jadwal pada waktu yang dipilih sudah ada.');
            }
    ...
    public function update(UpdateScheduleRequest $request, Schedule $schedule)
    {
        ...
        try {
            ...
            DB::commit();
            return redirect()->route('admin.master_data.schedule.index')->with('success', 'Jadwal berhasil diperbarui.');
        } catch (Exception $e) {
            DB::rollBack();
    ...
    {
        $day = $request->get('day', now()->format('Y-m-d')); // Default hari ini
        $classId = $request->get('class_id');

        $query = Schedule::with(['course', 'room', 'lecturer'])
            ->where('schedule_day', $day);

        if(auth()->user()->hasRole('Lecturer')) {
            $query = $query->where("nip", auth()->user()->lecturer->nip);
        } else {
            $query = $query->where("id_student_class", auth()->user()->student->studentClass->id_student_class);
        }

        if ($classId) {
            $query->where('id_student_class', $classId);
        }

        $schedules = $query->get();

        return ['html' => view('lecturer.task.schedule_card', compact('schedules'))->render()];
    }
}
